...
This metric quantifies the distance of each observation from the centroid of the data distribution (the mean vector), while accounting for the covariance structure among variables.
...
2.  Calculates the Mahalanobis distance for each sample using the first 10 PCs (I suggest the first 10 because they are the ones that PLINK saves by default).
3.  Sets the thresholds based on the chi-square distribution, specifically the quantile and the number of degrees of freedom. For convenience, we selected a quantile of 0.999 and set the number of degrees of freedom to equal the number of principal components.
